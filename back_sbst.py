...
def Load_HeNe(datafilename):
    ...
    x_interp = np.linspace(wnc.min(),wnc.max(),10000)
    wncx = HeNe_hacho/hikari * x * 1e12
    fringe_pos = interpolate_akm(wncx,wnc,x_interp)
    #-------------------------------------------------------#
    return fringe_pos

def Load_Data(signal,fringe,data_points_of_signals): 
    #signalはlist['str',...]で渡そう．fringeは１個で十分
    #data pointsは基本的に10000
    sig = np.zeros((len(signal),data_points_of_signals),dtype = float)
    i = 0
    for name in signal:
        a = loadtxt(name,1,None)
        sig[i] = a[0]
        i = i + 1
    fringe_pos_L = Load_HeNe(fringe)

    return sig, fringe_pos_L
...
#-------def---******************************************************************
#%%
x = np.linspace(-3.0,10,10000)
y_1 = erf_modoki(x,-0.1,0.1)

y_3 = -3*np.exp(-(x/0.8))
y_4 = 2.3*np.exp(-(x-0.1/1.0))
y_5 = 10*np.exp(-(x+0.1/0.01))

y_ex = (y_3 + y_4) * y_1 
y_chb = kink(x,-0.7,0.1,0.05)
...
def T(omg):
    return 1 / (omg * 1e12 ) * 1e+15

print(T(omg1)*10/T(omg1))
print(T(omg2)*10/T(omg1))
print(T(omg3)*10/T(omg1))
print(T(omg4)*10/T(omg1))
#%%
b30mw_back = loadtxt('bmote30mw_back', 1, None)
print(b30mw_back[0])

#%%
signal = ['bmote30mw_back']
fringe = 'C3fringe00001'

b30mw, fringe_pos = Load_Data(signal,fringe,10000)
...
#%%
min_index = index_seek(b30mw.min,b30mw)
delay = fringe_pos - fringe_pos[min_index]
