...
def loadtxt(datafilename,skip_rows):
    #csvの読み込みを行う関数，ロードしたいdatafilename(拡張子前)とskipしたい行数を右側に入れてね
    a = "./" + datafilename + ".txt" 
    df = np.loadtxt(a,skiprows = skip_rows)
    dftr = df.T
    return dftr
...
def FFT_abs_display(FFTsignal,frqaxis):
    ...
    if FFTsignal_len == frqaxis_len :
        ...
        print('both of elements number is not corresponding!!')
...
#%%
data = loadtxt("bmote30mw_back",1)
rb30mw = data[0]
delay = data[1]
print(len(rb30mw))
delayFT = delay_recalibration(delay,8192)
rb30mw_calb = interpolate_akm(rb30mw,delay,delayFT)

#%%
plt.plot(delay,rb30mw)
